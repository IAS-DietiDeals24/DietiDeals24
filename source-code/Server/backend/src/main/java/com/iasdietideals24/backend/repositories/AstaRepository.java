...
public interface AstaRepository extends CrudRepository<Asta, Long>, PagingAndSortingRepository<Asta, Long> {
    ...
    @Query(value = "select a from asta a where upper(a.nome) like ?1 and a.categoria.nome like ?2")
    Page<Asta> findByNomeLikeAndCategoria_NomeLike(String nomeAsta, String nomeCategoria, Pageable pageable);
