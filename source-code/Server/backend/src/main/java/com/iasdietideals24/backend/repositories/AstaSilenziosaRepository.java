...
public interface AstaSilenziosaRepository extends CrudRepository<AstaSilenziosa, Long>, PagingAndSortingRepository<AstaSilenziosa, Long> {
    ...
    @Query(value = "select a from asta_silenziosa a where upper(a.nome) like ?1 and a.categoria.nome like ?2")
    Page<AstaSilenziosa> findByNomeLikeAndCategoria_NomeLike(String nomeAsta, String nomeCategoria, Pageable pageable);
