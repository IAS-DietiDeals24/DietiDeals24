...
public interface AstaInversaRepository extends CrudRepository<AstaInversa, Long>, PagingAndSortingRepository<AstaInversa, Long> {
    ...
    @Query(value = "select a from asta_inversa a where upper(a.nome) like ?1 and a.categoria.nome like ?2")
    Page<AstaInversa> findByNomeLikeAndCategoria_NomeLike(String nomeAsta, String nomeCategoria, Pageable pageable);
